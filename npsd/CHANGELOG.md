...
- Added connection-layer single-port multiplexing so the existing HTTP control API and native NCP share the configured `17433` endpoint without consuming each other's bytes.
- Added the npsd local-dev native NCP session path: bounded RFC-0001 preamble and Hello reads, deterministic Hello/Caps negotiation, negotiated frame limits, canonical AnchorFrame ACK/cache, protocol ErrorFrame close, and silent pre-admission close.
- Added real-socket daemon tests and a strict Node L1 implementation manifest. Two NCP cases are verified and two remain partial; full L1 certification is explicitly not claimed.
- Expanded that manifest to enumerate all 20 L1 cases and embed its runnable evidence gates; incomplete required cases remain explicit rather than omitted.
- Replaced the process-local inbox with durable SQLite state. Undelivered ActionFrames, priority ordering, absolute TTL and acknowledgements now survive host restart; resident/hybrid push is explicitly declined for this L1 profile.
- Added publisher-signed ephemeral AnnounceFrame emission for npsd-managed agents, including persistent monotonic graph sequences, bounded heartbeat/TTL, graceful offline signals, retry isolation when the registry is unavailable, and conformance tests that reject signed-body tampering.
- Added bounded sub-NID renewal with atomic serial/validity replacement and fail-closed too-early, revoked, expired and concurrent-change paths. Managed agent private keys are retained only as host-root-bound AES-256-GCM envelopes so announcement signing survives restart; BYO keys remain outside npsd and are not auto-announced.
- Repaired both Docker build paths by using the .NET base image's non-root `app` identity, including the previously omitted observability project/source in the monorepo/overlay contexts, carrying native/conformance sources into standalone materialization, and replacing the unavailable `wget` health probe with npsd's built-in `--healthcheck` mode.
